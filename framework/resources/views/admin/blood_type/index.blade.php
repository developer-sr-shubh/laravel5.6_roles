...
$redirect = url('/').'/admin/blood_type?'.urlencode($_SERVER["QUERY_STRING"]);
...
<div class="right_col" role="main">
  <div class="row x_panel">
    ...
    <div class="pull-left">
      <a href="<?php echo url('/'); ?>/admin/blood_type/add" class="btn btn-success">Add Blood_type</a>
    </div>
    <br>
    <hr>
    ...
    <form method="GET" action="" class="form-inline ibox-content">
        <div class="form-group">
          <select name="searchBy" class="form-control">
           <option value="blood_type.blood_group" <?php echo $searchBy=="blood_type.blood_group"?'selected="selected"':""; ?>>Blood_group</option><option value="blood_type.status" <?php echo $searchBy=="blood_type.status"?'selected="selected"':""; ?>>Status</option>
          </select>
        </div>
        <div class="form-group">
    ...
    <div class="table-responsive">
      ...
      <th><input onclick="toggle(this,'cbgroup1')" id="foo[]" name="foo[]" type="checkbox" value="" /></th>
       <th> Sr No. </th>
          <?php $sortSym=isset($_GET["order"]) && $_GET["order"]=="asc" ? "up" : "down"; ?>
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="blood_type.blood_group"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["blood_type.blood_group_link"]; ?>" class="link_css"> Blood_group <?php echo $symbol ?></a>
            </th>
			
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="blood_type.status"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["blood_type.status_link"]; ?>" class="link_css"> Status <?php echo $symbol ?></a>
            </th>
      ...
      <?php $count=1;
          ...
          <tr id="hide<?php $value->id; ?>" >
            ...
            <th>
            <a href="<?php echo url("/")?>/admin/blood_type/status/status/<?php echo $value->id; ?>">
		   ...
		   <th>
           <a href="<?php echo url("/"); ?>/admin/blood_type/view/<?php echo $value->id?>" title="View">
            <span class="btn btn-info " ><i class="fa fa-eye"></i></span>
           </a>
           <a href="<?php echo url("/"); ?>/admin/blood_type/edit/<?php echo $value->id; ?>" title="Edit">
...
<div class="modal fade" id="commonDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 <form action="<?php echo url('/'); ?>/admin/blood_type/delete" method="post">
...
<script type="text/javascript">
   ...
   {var confrm = confirm("Are you sure you want to delete?");
     if(confrm)
     {
       ...
       $.ajax({
         type:"POST",
         url:'<?php echo url("/")."/admin/blood_type/deleteAll"; ?>',
         data:{
          allIds:ids,
          _token:"{{ csrf_token() }}",
         },
