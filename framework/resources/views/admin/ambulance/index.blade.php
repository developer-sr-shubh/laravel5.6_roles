...
$redirect = url('/').'/admin/ambulance?'.urlencode($_SERVER["QUERY_STRING"]);
...
<div class="right_col" role="main">
  <div class="row x_panel">
    ...
    <div class="pull-left">
      <a href="<?php echo url('/'); ?>/admin/ambulance/add" class="btn btn-success">Add Ambulance</a>
    </div>
    <br>
    <hr>
    ...
    <form method="GET" action="" class="form-inline ibox-content">
        <div class="form-group">
          <select name="searchBy" class="form-control">
           <option value="ambulance.ambulance_contact" <?php echo $searchBy=="ambulance.ambulance_contact"?'selected="selected"':""; ?>>Ambulance_contact</option><option value="ambulance.status" <?php echo $searchBy=="ambulance.status"?'selected="selected"':""; ?>>Status</option>
          </select>
        </div>
        <div class="form-group">
    ...
    <div class="table-responsive">
      ...
      <th><input onclick="toggle(this,'cbgroup1')" id="foo[]" name="foo[]" type="checkbox" value="" /></th>
       <th> Sr No. </th>
          <?php $sortSym=isset($_GET["order"]) && $_GET["order"]=="asc" ? "up" : "down"; ?>
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="ambulance.ambulance_contact"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["ambulance.ambulance_contact_link"]; ?>" class="link_css"> Ambulance_contact <?php echo $symbol ?></a>
            </th>
			
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="ambulance.status"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["ambulance.status_link"]; ?>" class="link_css"> Status <?php echo $symbol ?></a>
            </th>
      ...
      <?php $count=1;
          ...
          <tr id="hide<?php $value->id; ?>" >
            ...
            <th>
            {{{ $value->ambulance_contact }}}
            </th>
                
            <th>
            <a href="<?php echo url("/")?>/admin/ambulance/status/status/<?php echo $value->id; ?>">
		   ...
		   <th>
           <a href="<?php echo url("/"); ?>/admin/ambulance/view/<?php echo $value->id?>" title="View">
            <span class="btn btn-info " ><i class="fa fa-eye"></i></span>
           </a>
           <a href="<?php echo url("/"); ?>/admin/ambulance/edit/<?php echo $value->id; ?>" title="Edit">
...
<div class="modal fade" id="commonDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 <form action="<?php echo url('/'); ?>/admin/ambulance/delete" method="post">
...
<script type="text/javascript">
   ...
   {var confrm = confirm("Are you sure you want to delete?");
     if(confrm)
     {
       ...
       $.ajax({
         type:"POST",
         url:'<?php echo url("/")."/admin/ambulance/deleteAll"; ?>',
         data:{
          allIds:ids,
          _token:"{{ csrf_token() }}",
         },
