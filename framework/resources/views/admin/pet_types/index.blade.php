...
$redirect = url('/').'/admin/pet_types?'.urlencode($_SERVER["QUERY_STRING"]);
...
<div class="right_col" role="main">
  <div class="row x_panel">
    ...
    <div class="pull-left">
      <a href="<?php echo url('/'); ?>/admin/pet_types/add" class="btn btn-success">Add Pet_types</a>
    </div>
    <br>
    <hr>
    ...
    <form method="GET" action="" class="form-inline ibox-content">
        <div class="form-group">
          <select name="searchBy" class="form-control">
           <option value="pet_types.pet_type" <?php echo $searchBy=="pet_types.pet_type"?'selected="selected"':""; ?>>Pet_type</option><option value="pet_types.status" <?php echo $searchBy=="pet_types.status"?'selected="selected"':""; ?>>Status</option>
          </select>
        </div>
        <div class="form-group">
    ...
    <div class="table-responsive">
      ...
      <th><input onclick="toggle(this,'cbgroup1')" id="foo[]" name="foo[]" type="checkbox" value="" /></th>
       <th> Sr No. </th>
          <?php $sortSym=isset($_GET["order"]) && $_GET["order"]=="asc" ? "up" : "down"; ?>
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="pet_types.pet_type"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["pet_types.pet_type_link"]; ?>" class="link_css"> Pet_type <?php echo $symbol ?></a>
            </th>
			
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="pet_types.status"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["pet_types.status_link"]; ?>" class="link_css"> Status <?php echo $symbol ?></a>
            </th>
      ...
      <?php $count=1;
          ...
          <tr id="hide<?php $value->id; ?>" >
            ...
            <th>
            <a href="<?php echo url("/")?>/admin/pet_types/status/status/<?php echo $value->id; ?>">
		   ...
		   <th>
           <a href="<?php echo url("/"); ?>/admin/pet_types/view/<?php echo $value->id?>" title="View">
            <span class="btn btn-info " ><i class="fa fa-eye"></i></span>
           </a>
           <a href="<?php echo url("/"); ?>/admin/pet_types/edit/<?php echo $value->id; ?>" title="Edit">
...
<div class="modal fade" id="commonDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 <form action="<?php echo url('/'); ?>/admin/pet_types/delete" method="post">
...
<script type="text/javascript">
   ...
   {var confrm = confirm("Are you sure you want to delete?");
     if(confrm)
     {
       ...
       $.ajax({
         type:"POST",
         url:'<?php echo url("/")."/admin/pet_types/deleteAll"; ?>',
         data:{
          allIds:ids,
          _token:"{{ csrf_token() }}",
         },
